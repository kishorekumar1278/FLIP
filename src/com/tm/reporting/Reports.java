...
public class Reports extends UtilitiesLib{
	...
	{		
		...
		File f=new File(System.getProperty("user.dir")+"\\Results\\"+sdf.format(new Date()).replace("/", "_")+"\\TestSummaryReport");
				
		if(!f.exists())
		{			
			f.mkdirs();
			
			path=System.getProperty("user.dir")+"\\Results\\"+sdf.format(new Date()).replace("/", "-")+"\\TestSummaryReport";
		}else
			 {
				path=System.getProperty("user.dir")+"\\Results\\"+sdf.format(new Date()).replace("/", "-")+"\\TestSummaryReport";
			}		
